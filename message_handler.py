...
class MessageHandler:
    ...
    # Update the dictionary with a new code:pdata pair to send at the end of the server loop.
    ...
    # Unpack the pdata from message_queue, receive the user_id:socket dictionary and dish it all out!
        ...
        for data_to_send in self.message_queue:
            ...
            if client_code == ALL_CODE:
                for client in client_ids:  # Loop over the keys in the bidirectional dictionary
                    if isinstance(client, socket.socket):  # Only try sending pdata for the sockets
                        try:
                            client.sendall(data_to_send)
                        except Exception as e:
                            print(f"Error sending pdata to socket: {e}")  # todo: remove this and handle the socket closing properly
